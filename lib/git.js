...
var validTypes = require('./writer').types
...
exports.getCommits = function (options) {
  ...
  .map(function (raw) {
    ...
    if (parsed[1].includes("merge branch")) {
      console.log('>>> hit')
      commit.type = 'other'
      return commit
    }
    ...
    if (!Object.keys(validTypes).includes(commit.type)) {
  ...
  .filter(function (commit) {
    ...
    var shouldNotExclude = (options.exclude.indexOf(commit.type) === -1) && (Object.keys(validTypes).includes(commit.type))
    
    return options.exclude ? shouldNotExclude : true;
  });
};
